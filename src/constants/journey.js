...
// Atmospheric distance for drifters: how fast far bodies dim and desaturate
// per unit of z beyond 1, with floors so nothing fades away entirely.
export const DEPTH_HAZE = { dim: 0.18, dimMin: 0.62, desat: 0.14, desatMin: 0.7 }

// Bodies drifting past along the route — the near field that sells the flight,
// like everswap's foreground peaks. Each body flies in perspective: its lateral
// offset (from → to, in vw/vh at distance 1) and its size are divided by the
// current distance `z`, so an approach accelerates hyperbolically — slow while
// far, whipping past the frame edge up close.
//   window   [anchor, offsetVh] pair — anchors are section edges, resolved at
//            measure time: top / workTop / workBottom / lifeTop / lifeBottom /
//            arrivalTop
//   z        [far, near] distance across the window (omit for a flat crossing)
//   kind     'planet' (PixelPlanet: palette + fixed spin + scale) or
//            'asteroid' (PixelAsteroid: size in vmin + rotTurns tumble)
// Every body draws once per visit — only transforms move, so the field is free.
export const DRIFTERS = [
	{
		// the rock we launch past: parked in the lower-right of the hero at rest,
		// it swells and sweeps off the corner as the journey begins
		kind: 'asteroid',
		size: 26,
		rotTurns: 0.2,
		window: [
	...
	{
		// far sister world — approaches only slightly, because distance
		kind: 'planet',
		palette: 'ice',
		spin: 4.2,
		scale: 0.42,
	...
	{
		// the moon slides across overhead during the skim — orbital lateral
		// motion plus a shallow approach
		kind: 'planet',
		palette: 'moon',
		spin: 2.1,
		scale: 0.42,
	...
	{
		// threading the belt on the way down: a near rock blows past on the left…
		kind: 'asteroid',
		size: 24,
		rotTurns: -0.35,
		window: [
	...
	{
		// …while its small partner hangs farther out in the stream
		kind: 'asteroid',
		size: 18,
		rotTurns: 0.3,
		window: [
